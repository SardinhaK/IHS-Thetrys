...
shape_colors = [(25, 160, 50), (55, 100, 60), (255, 0, 50), (160, 255, 0), (0, 180, 255), (0, 0, 255), (140, 220, 255) , (255,150,220)]
...
#CONVERTENDO O FORMATO DO OBJETO
...
def clear_rows(grid, locked, score, fd):
	...
	for i in range(len(grid) - 1, -1, -1):
		...
		if (255, 255, 255) not in row:
			...
			data = data + 0x40400000
			retval = os.write(fd, data.to_bytes(4, 'little'))
			global scoreMAX
			if score > scoreMAX:
				scoreMAX = score
				print("Novo scoreMAX: ", scoreMAX)
...
def draw_right_side(shape, surface, score):
    ...
    label2 = font.render(str(score*10), 1, (255, 255, 255))
...
def desenhar_janela(surface):
	...
	label = font.render('THETRYS', 1, (255, 0, 255))
...
def main():
    ...
    while run:
        ...
        if change_piece:
            ...
            Score[0] = clear_rows(grid, locked_positions, Score[0], fd)
		
        desenhar_janela(win)
        draw_right_side(next_piece, win, Score[0])
        pygame.display.update()

        if check_lost(locked_positions):
        	print("checou se perdeu")
        	run = False
        	
            
    draw_text_middle("PONTUAÇÃO MÁXIMA : " + str(Score[0]), 40, (0, 0, 0), win)
    pygame.display.update()
    data2 = 0b111000111000111000
    data1 = 0b000111000111000111  
    ioctl(fd, WR_GREEN_LEDS)
    data = 0b0
    retval = os.write(fd, data.to_bytes(4, 'little'))
    ioctl(fd, WR_RED_LEDS)
...
def main_menu():
    ...
    while run:
        ...
        win.fill(color)
        draw_main_menu(70,(60,155,25),win)
        pygame.display.update()
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False
...
pygame.display.set_caption('T H E T R Y S')
